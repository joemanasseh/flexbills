...
class BuyerPaymentController extends GetxController with BuyerPaymentApiService {
  ...
  Future<BuyerPaymentIndexModel> buyerPaymentIndexFetch() async {
    ...
    await buyerPaymentIndexAPi(Get.find<MyEscrowController>().escrowData.id.toString()).then((value) {
  ...
  _onPayment(BuildContext context) async {
    ...
    if (selectedPaymentMethodType.value == "AUTOMATIC") {
      if (selectedPaymentMethodTypeId.value == "1") {
        await buyerPaymentPaypal().then((value) {
          if(value != null) {
            _webPayment(context,
        ...
        await buyerPaymentAutomatic().then((value) {
          if(value != null) {
            _webPayment(context,
    ...
    /// for manual payment
      ...
      await buyerPaymentManualPayment().then((value) {
        if(value!= null){
  ...
  Future<CommonSuccessModel> buyerPaymentConfirm() async {
    ...
    Map<String, String> inputBody = {
      "payment_gateway": "myWallet"
    };

    await buyerPaymentConfirmApi(buyerPaymentIndexModel.data.escrowInformation.id.toString(), body: inputBody).then((value) {
  ...
  Future<EscrowPaypalPaymentModel?> buyerPaymentPaypal() async {
    _isSubmitLoading.value = true;
    update();
    ...
    await buyerPaymentPaypalApi(buyerPaymentIndexModel.data.escrowInformation.id.toString(), body: inputBody).then((value) {
  ...
  Future<EscrowAutomaticPaymentModel?> buyerPaymentAutomatic() async {
    _isSubmitLoading.value = true;
    update();
    ...
    await buyerPaymentAutomaticApi(buyerPaymentIndexModel.data.escrowInformation.id.toString(), body: inputBody).then((value) {
  ...
  Future<EscrowManualPaymentModel?> buyerPaymentManualPayment() async {
    ...
    await buyerPaymentManualApi(buyerPaymentIndexModel.data.escrowInformation.id.toString(), body: inputBody).then((value) {
