...
class AddNewEscrowController extends GetxController with EscrowApiService {
  ...
  Future<EscrowCreateModel> escrowIndexFetch() async {
    ...
    await escrowCreateAPi().then((value) {
      ...
      selectedMyRole.value = _escrowIndexModel.data.userType == "seller"
          ? "Seller"
          : "Buyer";
      selectedOppositeRole.value = _escrowIndexModel.data.userType == "seller"
          ? "Buyer"
          : "Seller";
    ...
    if (formKey.currentState!.validate()) {
      if (selectedFilePath.value.isEmpty) {
        await escrowWithoutFileSubmit()
            .then((value) => Get.toNamed(Routes.addNewEscrowPreviewScreen));
      } else {
  ...
  _onPayment(BuildContext context) async {
    ...
    if (selectedPaymentMethodType.value == "AUTOMATIC") {
      if (selectedPaymentMethodTypeId.value == "1") {
        ...
        await escrowAutomatic().then((value) {
          if(value != null) {
            _webPayment(context,
    ...
    /// for manual payment
      ...
      await escrowManualPayment().then((value) {
        if(value!= null){
  ...
  Future<EscrowPaypalPaymentModel?> escrowPaypal() async {
    _isSubmitLoading.value = true;
    update();
  ...
  Future<EscrowAutomaticPaymentModel?> escrowAutomatic() async {
    _isSubmitLoading.value = true;
    update();
